...
export class AdvancedService {
  ...
  get allproduct$() {
    return this._allproduct$.asObservable();
  }
  get product() {
  ...
  get ProductFilter() {
    return this._state.ProductFilter;
  }
  get productPrice() {
    return this._state.productPrice;
  }
  get productRate() {
    return this._state.productRate;
  }
  get productStatus() {
    return this._state.productStatus;
  }
  get startIndex() {
  ...
  set ProductFilter(ProductFilter: string) {
    this._set({ ProductFilter });
  }
  set productPrice(productPrice: number) {
    this._set({ productPrice });
  }
  set productStatus(productStatus: string) {
    this._set({ productStatus });
  }
  set productRate(productRate: number) {
    this._set({ productRate });
  }
  ...
  private _search(): Observable<SearchResult> {
    ...
    if (ProductFilter) {
      countries = countries.filter((country) =>
        matches(country, ProductFilter, this.pipe)
      );
    }
    ...
    if (productPrice) {
      countries = countries.filter(
        (country) =>
          country.price >= Object.values(productPrice)[0] &&
          country.price <= Object.values(productPrice)[1]
      );
    } else {
      countries = countries;
    }
    ...
    if (productStatus) {
      countries = countries.filter(
        (country) => country.status == productStatus
      );
    } else {
      countries = countries;
    }
