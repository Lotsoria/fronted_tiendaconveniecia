...
export class AdvancedService {
  ...
  get allfood$() {
    return this._allfood$.asObservable();
  }
  get food() {
    return this.foods;
  }
  ...
  get FoodFilter() {
    return this._state.FoodFilter;
  }
  get foodPrice() {
    return this._state.foodPrice;
  }
  get foodRate() {
    return this._state.foodRate;
  }
  get foodStatus() {
    return this._state.foodStatus;
  }
  get startIndex() {
  ...
  set FoodFilter(FoodFilter: string) {
    this._set({ FoodFilter });
  }
  set foodPrice(foodPrice: number) {
    this._set({ foodPrice });
  }
  set foodStatus(foodStatus: string) {
    this._set({ foodStatus });
  }
  set foodRate(foodRate: number) {
    this._set({ foodRate });
  }
  ...
  private _search(): Observable<SearchResult> {
    ...
    if (FoodFilter) {
      countries = countries.filter((country) =>
        matches(country, FoodFilter, this.pipe)
      );
    }
    ...
    if (foodPrice) {
      countries = countries.filter(
        (country) =>
          country.price >= Object.values(foodPrice)[0] &&
          country.price <= Object.values(foodPrice)[1]
      );
    } else {
      countries = countries;
    }
  ...
  getFoods() {
    ...
    const data = this.http.get<any>(`${URL_API}food/list`, {
