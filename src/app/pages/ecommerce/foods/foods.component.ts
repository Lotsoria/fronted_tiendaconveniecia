...
import { NgbdFoodsSortableHeader, foodSortEvent } from './foods-sortable.directive';
...
export class FoodsComponent {
  ...
  @ViewChildren(NgbdFoodsSortableHeader) headers!: QueryList<NgbdFoodsSortableHeader>;
  searchfoods: any;
  publishedfood: any;
    ...
    */
    ...
    setTimeout(() => {
      this.foodList.subscribe(x => {
        this.content = this.foods;
        this.foods = Object.assign([], x);
      });

      this.allfoodList.subscribe(x => {
        this.allfoods = Object.assign([], x);
      });
    ...
    setTimeout(() => {
      for (var i = 0; i < this.allfoods.length; i++) {
        if (this.allfoods[i].category == 'Kitchen Storage & Containers') {
          this.grocery += 1
        }
        if (this.allfoods[i].category == 'Clothes') {
          this.fashion += 1
        }
        if (this.allfoods[i].category == 'Watches') {
          this.watches += 1
        } if (this.allfoods[i].category == 'Electronics') {
          this.electronics += 1
        } if (this.allfoods[i].category == 'Furniture') {
          this.furniture += 1
        } if (this.allfoods[i].category == 'Bike Accessories') {
          this.accessories += 1
        }
        if (this.allfoods[i].category == 'Tableware & Dinnerware') {
          this.appliance += 1
        }
        if (this.allfoods[i].category == 'Bags, Wallets and Luggage') {
          this.kids += 1
        }
        if (this.allfoods[i].status == 'published') {
          this.totalpublish += 1
        }
      }
    }, 2000);
